...
printOutput(
  27,
  `Distinct elements from [1, 2, 2, 3, 4, 4]: ${distinctArray([
    1, 2, 2, 3, 4, 4,
  ])}`
);
...
printOutput(
  30,
  `Adding large numbers "123456789123456789" and "987654321987654321": ${addLargeNumbers(
    "123456789123456789",
    "987654321987654321"
  )}`
);
...
printOutput(
  31,
  `Number of words in "Hello world! This is a test.": ${wordCount(
...
printOutput(
  38,
  `String from ASCII codes [104, 101, 108, 108, 111]: "${asciiArrayToString([
    104, 101, 108, 108, 111,
  ])}"`
);
...
printOutput(
  39,
  `Caesar cipher of "abc" with shift 2: "${caesarCipher("abc", 2)}"`
);
...
printOutput(
  41,
  `Distance between points (1,2) and (4,6): ${distanceBetweenPoints(
    1,
    2,
    4,
    6
...
printOutput(
  42,
  `Do circles (0,0,1) and (1,1,1) intersect? ${circlesIntersect(
    0,
    0,
    1,
    1,
...
printOutput(
  43,
  `Extracted column 1 from [[1,2],[3,4]]: ${extractColumn(
    [
      [1, 2],
      [3, 4],
    ],
...
const sumJaggedArray = (arr) => {
  return arr.reduce(
    (sum, el) => sum + (Array.isArray(el) ? sumJaggedArray(el) : el),
    0
  );
};
printOutput(
  45,
  `Sum of jagged array [1,[2,[3]],4]: ${sumJaggedArray([1, [2, [3]], 4])}`
);

// 46. Find the maximum number in a jagged array of numbers or array of numbers
const maxInJaggedArray = (arr) => {
  return arr.reduce(
    (max, el) => Math.max(max, Array.isArray(el) ? maxInJaggedArray(el) : el),
    -Infinity
  );
};
printOutput(
  46,
  `Max in jagged array [[1,2],[3,4]]: ${maxInJaggedArray([
    [1, 2],
    [3, 4],
  ])}`
);
...
printOutput(
  49,
  `Merged objects {a:1} and {b:2}: ${JSON.stringify(
...
printOutput(
  50,
  `Switched keys and values of {a:1, b:2}: ${JSON.stringify(
    switchKeysAndValues({ a: 1, b: 2 })
  )}`
);
...
printOutput(
  51,
  `Count items in object {a:1, b:{c:2}}: ${countItemsInObject({
    a: 1,
    b: { c: 2 },
  })}`
);
...
printOutput(
  52,
  `Common elements between [1,2,3] and [2,3,4]: ${commonElements(
    [1, 2, 3],
    [2, 3, 4]
  )}`
);
...
printOutput(
  53,
  `Unique elements between [1,2,3] and [2,3,4]: ${uniqueToOneArray(
    [1, 2, 3],
    [2, 3, 4]
  )}`
);
